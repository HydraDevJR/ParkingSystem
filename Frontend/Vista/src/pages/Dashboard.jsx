...
>>>>>>> 500bae1edff157b94916672b542e49f3f816d05c
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                // Estadías activas: estado "activa" o sin horaSalida
                const activas = estadias.filter(
                    (e) => e.estado === "activa" || !e.horaSalida,
                );
                ...
                const ingresosHoy = estadias
                    .filter(
                        (e) =>
                            e.horaSalida && new Date(e.horaSalida).toDateString() === hoy,
                    )
                    .reduce((sum, e) => sum + (e.valorPagado ?? 0), 0);
                ...
                const ingresosMes = estadias
                    .filter((e) => {
                        if (!e.horaSalida) return false;
                        const fecha = new Date(e.horaSalida);
                        return (
                ...
                // Estadías recientes (las últimas 5 por horaEntrada)
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <p className="mt-2 text-xs text-slate-500">
                        De {resumenCeldas.total} totales
                    </p>
                </div>
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <p className="text-slate-600 text-xs">
                                                Entrada: {formatDateTime(estadia.horaEntrada)}
                                            </p>
                                        </div>
                                    </div>
                                    <p className="text-[#0A2647] text-sm font-bold">
                                        {formatCurrency(estadia.valorPagado ?? estadia.valor)}
                                    </p>
                                </div>
                            ))
                        )}
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        <Link
                            to="/dashboard/registro-entrada"
                            className="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
                        >
                            Registrar entrada
                        </Link>
                        <Link
                            to="/dashboard/registro-salida"
                            className="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
                        >
                            Registrar salida
                        </Link>
                        <Link
                            to="/dashboard/ver-mapa-celdas"
                            className="inline-flex items-center justify-center rounded-lg bg-[#0A2647] px-4 py-3 text-sm font-bold text-white hover:bg-[#0A2647]/90 transition-colors"
                        >
                            Ver mapa de celdas
                        </Link>
                    </div>
